...
export class LogisticsService {
  ...
  async updateStoreProfile(
    ...
    if (body.defaultProvider === "GIGL" && !body.giglStationId) {
      throw new LogisticsError(
        "GIGL_STATION_REQUIRED",
        "A GIGL sender station is required when GIGL is the default provider.",
        400,
      );
    }
    if (body.defaultProvider === "MANUAL" && !body.manualDeliveryEnabled) {
      throw new LogisticsError(
        "MANUAL_DELIVERY_DISABLED",
        "Manual delivery cannot be the default while it is disabled.",
        400,
      );
    }
  ...
  async quoteCart(
    principal: AccessPrincipal,
    body: ShippingQuoteRequestDto,
  ): Promise<ShippingQuoteResponseDto> {
    const cart = await this.cartService.getCart(principal);
    if (cart.items.length === 0) {
      throw new LogisticsError("CART_EMPTY", "The active cart is empty.", 409);
    }
    ...
    for (const [storeId, items] of groups) {
      ...
      if (!profile || !profile.active) {
        throw new LogisticsError(
          ...
          `Store ${storeId} has no active fulfillment profile.`,
          409,
        );
      }
      ...
      if (profile.defaultProvider === "MANUAL") {
        if (!profile.manualDeliveryEnabled || profile.manualDeliveryFeeAmountMinor === null) {
          throw new LogisticsError(
            "MANUAL_DELIVERY_NOT_CONFIGURED",
            `Store ${storeId} has no manual delivery fee.`,
            409,
          );
        }
          ...
          quotes: [
            {
              ...
              storeId,
              provider: "MANUAL",
              amount: { amountMinor: quote.amountMinor.toString(), currency: quote.currency },
              serviceCode: "MANUAL",
      ...
      if (!body.receiverStationId || !profile.giglStationId) {
        throw new LogisticsError(
          "GIGL_STATION_REQUIRED",
          "Sender and receiver GIGL stations are required for this quote.",
          400,
        );
      }

      const shippingProfiles = await this.repository.listVariantProfiles(
      ...
      const quoteItems: LogisticsQuoteItem[] = items.map((item) => {
        ...
        if (!shipping) {
          throw new LogisticsError(
            "VARIANT_SHIPPING_PROFILE_REQUIRED",
            `Variant ${item.variant.id} has no shipping profile.`,
            409,
          );
        }
        return {
      ...
      const providerQuote = await adapter.quote({
        ...
        items: quoteItems,
        declaredValueMinor: items.reduce(
          (sum, item) => sum + BigInt(item.lineSubtotal.amountMinor),
          0n,
        ),
        currency: profile.currency,
      });
      const quote = await this.repository.createQuote({
        ...
        ...(providerQuote.serviceCode ? { serviceCode: providerQuote.serviceCode } : {}),
        ...(providerQuote.providerQuoteReference
          ? { providerQuoteReference: providerQuote.providerQuoteReference }
          : {}),
        requestFingerprint: fingerprint,
        expiresAt,
        ...(providerQuote.metadata ? { metadata: providerQuote.metadata as never } : {}),
      });
      result.push({
        storeId,
        quotes: [
  ...
  async createShipment(
    ...
    if (context.paymentStatus !== "SUCCEEDED") {
      throw new LogisticsError(
        "PAYMENT_REQUIRED",
        "Shipment creation requires verified payment.",
        409,
      );
    }
    if (["CANCELLED", "REFUNDED"].includes(context.status)) {
      throw new LogisticsError(
        "VENDOR_ORDER_NOT_FULFILLABLE",
        "This vendor order cannot be fulfilled.",
        409,
      );
    }
    ...
    for (const requested of body.items) {
      ...
      if ((allocated.get(item.id) ?? 0) + requested.quantity > item.quantity) {
        throw new LogisticsError(
          ...
          `Shipment quantity exceeds remaining quantity for order item ${item.id}.`,
          409,
        );
      }
    }
  ...
  async recordManualStatus(
    ...
    if (shipment.provider !== "MANUAL") {
      throw new LogisticsError(
        "MANUAL_STATUS_ONLY",
        "Only manual shipments accept vendor-entered status updates.",
        409,
      );
    }
    const context = await this.orderBoundary.getVendorOrderContext(shipment.vendorOrderId);
    ...
    await this.vendorBoundary.requireStorePermission(principal, context.storeId, "order:fulfill");
    if (!canTransitionManualShipment(shipment.status, status)) {
      throw new LogisticsError(
        "INVALID_SHIPMENT_TRANSITION",
        `Shipment cannot transition from ${shipment.status} to ${status}.`,
        409,
      );
    }

    const changed = await this.repository.appendShipmentEvent({
      shipmentId,
      status,
      ...(message ? { message } : {}),
      eventTime: new Date(),
    ...
    await this.orderBoundary.applyShipmentAggregate(
      shipment.vendorOrderId,
      siblings.map((entry) => (entry.id === changed.id ? changed.status : entry.status)),
      new Date(),
    );
    ...
    for (const shipment of await this.repository.listTrackingCandidates(limit)) {
      ...
      try {
        ...
        await this.repository.appendShipmentEvent({
          ...
          ...(tracking.message ? { message: tracking.message } : {}),
          ...(tracking.location ? { location: tracking.location } : {}),
          eventTime: tracking.eventTime,
          ...(tracking.metadata ? { metadata: tracking.metadata as never } : {}),
        });
        const siblings = await this.repository.listVendorOrderShipments(shipment.vendorOrderId);
        await this.orderBoundary.applyShipmentAggregate(
          shipment.vendorOrderId,
          siblings.map((entry) =>
            entry.id === shipment.id ? tracking.status : entry.status,
          ),
          new Date(),
        );
        changed += 1;
      } catch {
        // Provider failure is retryable. The last trusted shipment state remains unchanged.
      }
    }
    return changed;
  }
...
export class LogisticsAwareCheckoutFinancialPolicy implements CheckoutFinancialPolicy {
  ...
  async quoteStore(input: CheckoutStorePolicyInput): Promise<CheckoutStoreFinancialQuote> {
    ...
    return {
      ...base,
      deliveryAmountMinor: await this.logistics.resolveCheckoutDelivery(input),
    };
  }
}
