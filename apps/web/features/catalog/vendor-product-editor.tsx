...
function VariantEditor({
  ...
  return (
    <tr>
      ...
      <td>{canUpdate ? <button className="secondaryButton compactButton" type="button" disabled={busy} onClick={() => void save()}>{busy ? "Saving…" : "Save"}</button> : null}{message ? <small className="tableMessage">{message}</small> : null}</td>
    </tr>
  );
}
  ...
  async function save() {
    ...
    try {
      await catalogApi.updateMedia(media.id, {
        ...
        displayOrder: Number.parseInt(displayOrder || "0", 10),
      });
      await onChanged();
  ...
  return (
    <article className="mediaEditorCard">
      ...
      <div className="mediaEditorBody">
        ...
        <label className="field">Display order<input type="number" value={displayOrder} onChange={(event) => setDisplayOrder(event.target.value)} disabled={!canUpdate || busy} /></label>
...
export function VendorProductEditor({ vendorId, productId }: Readonly<{ vendorId: string; productId: string }>) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (nextProduct.storeId && access.vendor.id !== vendorId) throw new Error("Vendor route mismatch.");
      ...
      setMessage(apiErrorMessage(caught, "CartNest could not load this product."));
      setState("error");
    }
  ...
  if (!canRead) return <ErrorState title="Product access restricted" message="Your vendor membership does not include product:read." />;
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await load();
      setMessage("Product image uploaded and verified.");
    } catch (caught) {
      ...
      <VendorWorkspaceShell access={access}>
        ...
        <section className="productEditorStack">
          <form className="panel sellerForm" onSubmit={saveMetadata}>
            <div className="sectionHeadingCompact"><div><h2>Product details</h2><p>If a moderated product changes, CartNest now returns it to PENDING instead of preserving stale approval.</p></div><Link className="textButton" href={`/products/${encodeURIComponent(product.id)}`}>View public page</Link></div>
          ...
          <section className="panel sellerForm">
            ...
            <div className="variantCreateTableWrap">
              <table className="dataTable variantCreateTable"><thead><tr><th>Variant</th><th>SKU</th><th>NGN price</th><th>Status</th><th>Action</th></tr></thead><tbody>{product.variants.map((variant) => <VariantEditor key={variant.id} variant={variant} canUpdate={canUpdate} onSaved={(updated) => setProduct((current) => current ? { ...current, variants: current.variants.map((item) => item.id === updated.id ? updated : item) } : current)} />)}</tbody></table>
            </div>
          ...
          <section className="panel sellerForm">
            <div className="sectionHeadingCompact"><div><h2>Product images</h2><p>JPEG, PNG and WebP up to 10 MB. The API now verifies actual image signatures after the direct R2 upload.</p></div></div>
            ...
            <div className="mediaEditorGrid">{product.media.filter((media) => media.status !== "DELETED").map((media) => <MediaCard key={media.id} media={media} canUpdate={canUpdate} onChanged={load} />)}</div>
          </section>
        </section>
